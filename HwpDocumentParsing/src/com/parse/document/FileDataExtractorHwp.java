...
public abstract class FileDataExtractorHwp extends FileDataExtractor {
	...
	public void processDocument(IProgressMonitor monitor, DataExtractContext context, String title,
			DocumentExtractorHwp documentExtractor) throws Exception {
			...
			if (hwpFiles != null) {
				...
				for (HWPFile hwpFile : hwpFiles) {
					if (hwpFile != null) {
						...
						allFilesParagraphs.add(paragraphs); // 각 파일의 문단 리스트를 모아둠
					}
				}

				documentExtractor.extractElement(monitor, context, title, allFilesParagraphs, false);
	...
	/**
	 * 다중 약관 일 때 처리하는 메서드
	 * 이전 문구가 "약관 목차"가 아닐 때의 "제1관 목적 및 용어의 정의"를 찾음
	 * 찾은 후 -3 또는 -2 번째에 있는 보종명을 가져와서 시작 부분을 설정
	 * 끝 부분은 "약관 목차"가 나올 때까지로 설정
	 * @param monitor
	 * @param context
	 * @param title
	...
	public void processDocumentEx(IProgressMonitor monitor, DataExtractContext context, String title,
	        DocumentExtractorHwp documentExtractor, String bojongName) throws Exception {
	    ...
	    if (multipleCheck && hwpFiles != null) {
	            ...
	            if (hwpFile != null) {
	                ...
	                for (int i = 0; i < paragraphs.size(); i++) {
	                    ...
	                    if (cleanedText.equals(FIND_FIRST_GAWN)) {
	                        if (i >= 3) {
	                            String precedingText = Util.replaceBlank(HwpUtil.extractParagraphText(paragraphs.get(i - 3), i, title));
	                            if (precedingText.isEmpty()) {
	                                precedingText = Util.replaceBlank(HwpUtil.extractParagraphText(paragraphs.get(i - 2), i, title));
	                            }

	                            String prevText = Util.replaceBlank(HwpUtil.extractParagraphText(paragraphs.get(i - 1), i, title));
	        ...
	        processDocument(monitor, context, title, documentExtractor);
	    }
